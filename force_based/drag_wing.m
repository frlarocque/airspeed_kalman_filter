...
for i=1:length(skew_bins)
    ...
    legend_lbl{i} = [mat2str(rad2deg(skew_bins(i))),' deg'];
    error_vec = [error_vec; temp_group.mean_Fx_scaled-fit_1(s_all_1,[temp_group.Turn_Table,ones(length(temp_group.Turn_Table),1).*1,ones(length(temp_group.Turn_Table),1).*skew_bins(i)])];
