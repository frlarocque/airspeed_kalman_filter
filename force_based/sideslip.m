...
test_db_2 = test_db_2(test_db_2.Mot_F==0 & test_db_2.Mot_R==0 & test_db_2.Mot_B==0 & test_db_2.Mot_L==0,:);
...
for i=1:length(windspeed_bins)
    ...
    legend_lbl{i} = [mat2str(windspeed_bins(i)),' m/s'];
    error_vec = [error_vec; -temp_db.Fy-fit_quad(s_quad,[temp_db.Turn_Table,ones(length(temp_db.Fy),1).*0,ones(length(temp_db.Fy),1).*windspeed_bins(i)])];
end
...
%% Nice plot for sideforce generated by wing
nice_db = test_db_2(test_db_2.Turn_Table==turn_table_bins(end-3),:);
...
for i=1:length(windspeed_bins)
    ...
    hdls(i,1) = plot(rad2deg(temp_db.Skew_sp),temp_db.Fy,mymarkerstyles{i},'color',col(i,:),'MarkerSize',marker_size);
    hold on
    
    plot(rad2deg(linspace(0,deg2rad(90),20)),cos(linspace(0,deg2rad(90),20)).^2.*sin(linspace(0,deg2rad(90),20)).*8E-2.*windspeed_bins(i).^2,'-','color',col(i,:))
    plot(rad2deg(linspace(0,deg2rad(90),20)),cos(linspace(0,deg2rad(90),20)).*sin(linspace(0,deg2rad(90),20)).*6E-2.*windspeed_bins(i).^2,'--','color',col(i,:))

    legend_lbl{i} = [mat2str(windspeed_bins(i)),' m/s'];
    
    error_vec = [error_vec;temp_db.Fy-cos(temp_db.Skew_sp).*sin(temp_db.Skew_sp).*6E-2.*windspeed_bins(i).^2];
    %error_vec = [error_vec;temp_db.Fy-cos(temp_db.Skew_sp).^2.*sin(temp_db.Skew_sp).*8E-2.*windspeed_bins(i).^2];
end
lgd1 = legend(hdls(:,1),legend_lbl,'Location', 'northoutside', 'Orientation', 'horizontal');
...
fig_name = ['WING_FY_',formattedDateTime,'.eps'];
exportgraphics(fig,fig_name,'BackgroundColor','none','ContentType','vector')
