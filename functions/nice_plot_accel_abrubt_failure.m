...
% tail
[xt yt] = ds2nfu(zoom_range(1)-6, 17.5);
% head
...
[p, z, d] = zoomPlot(kalman_res{1}.t,kalman_res{1}.x(1,:), zoom_range, [0.2 0.68 0.35 0.24],[2 3]);
xticks(zoom_range(1):1:zoom_range(2))
ylim([13.0 16.5])
d.Color = mycolors(2,:);
d.LineStyle = mylinestyles{2};
hold on
grid on
%grid minor
ax3=gca;
...
f = xline(fault_time,'-',{'Fault'},'linewidth',1.5*line_width,'color',mycolors(3,:),'LabelHorizontalAlignment','right','LabelVerticalAlignment','top','fontsize',font_size-4);
...
d1 = xline(detection_high,'--',{'Criteria Norm.'},'linewidth',1.5*line_width,'color',mycolors(4,:),'LabelHorizontalAlignment','left','LabelVerticalAlignment','top','fontsize',font_size-4);
