classdef EKF < handle
    ...
    methods
        ...
        function update_EKF(obj,u,z)
            ...
            L_val = state_noise_jacobian(obj,x,u);
            M_val = output_noise_jacobian(obj,x,u);
        
            % Prediction
            x_pred = x + obj.dt*obj.f_fh(x,u);   
            P_pred = F_val*P_last*F_val'+L_val*obj.Q*L_val';
        
            % Update
            obj.y_list(:,k) = z-obj.g_fh(x,u);
        
            %Kalman gain
            obj.K{k} = P_pred*G_val'*inv(G_val*P_pred*G_val'+M_val*obj.R*M_val');
        ...
        function M = output_noise_jacobian(obj,x,u)
            y = obj.g_fh(x,u);
            M = zeros(length(y),length(y));
            for i=1:length(y)
                
               w_p = zeros(size(y));w_p(i)=w_p(i)+obj.epsi;
               w_m = zeros(size(y));w_m(i)=w_m(i)-obj.epsi;
            
               dm = (obj.g_fh(x,u,w_p)-obj.g_fh(x,u,w_m))/(2*obj.epsi);
                
               M(:,i) = dm;
            end
        end

        function L = state_noise_jacobian(obj,x,u)
            noise_length = 9;

            L = zeros(length(x),noise_length);
            for i=1:noise_length
            
            w_p = zeros(noise_length,1);w_p(i)=w_p(i)+obj.epsi;
            w_m = zeros(noise_length,1);w_m(i)=w_m(i)-obj.epsi;
            
            dl = (obj.f_fh(x,u,w_p)-obj.f_fh(x,u,w_m))/(2*obj.epsi);
